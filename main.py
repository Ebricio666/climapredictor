...
#    return pd.DataFrame(datos_consolidados)
    ...
    try:
        ...
        if estaciones_disponibles:
            ...
            @st.cache_data
            def cargar_serie_diaria_estacion(clave, variable, output_dirs, elevation_data, tile_size):
                """
                Carga la serie DIARIA original de una estación para Prophet.

                A diferencia de df_consolidado_imputado (que resume por Año/Mes),
                esta función conserva la fecha real de cada observación. Esto permite
                solicitar una predicción para un día concreto.
                """
                for output_dir in output_dirs:
                    archivo = os.path.join(output_dir, f"{clave}_df.csv")
                    ...
                    # Variables observadas directamente en los CSV.
                    if variable in df.columns:
                        df[variable] = pd.to_numeric(
                            df[variable].astype(str).str.replace('[^0-9.-]', '', regex=True),
                            errors='coerce'
                        )
                        serie = df[['Fecha', variable]].rename(columns={'Fecha': 'ds', variable: 'y'})

                    # Para radiación solar se calcula una estimación diaria con la misma
                    # lógica física ya usada en la aplicación.
                    elif variable in ['Radiación Solar Promedio (W/m²)', 'Radiación Solar Corregida (W/m²)']:
                        if 'Latitud' not in df.columns or 'Longitud' not in df.columns:
                            return pd.DataFrame(columns=['ds', 'y'])

                        latitud = pd.to_numeric(df['Latitud'], errors='coerce').dropna()
                        longitud = pd.to_numeric(df['Longitud'], errors='coerce').dropna()
                        if latitud.empty or longitud.empty:
                            return pd.DataFrame(columns=['ds', 'y'])

                        lat = float(latitud.iloc[0])
                        lon = float(longitud.iloc[0])
                        alt = obtener_elevacion(lat, lon, tile_size, elevation_data)

                        S0 = 1361
                        Ta = 0.75
                        k = 0.12

                        def radiacion_diaria(fecha):
                            if pd.isna(fecha):
                                return np.nan
                            dia = int(fecha.dayofyear)
                            decl = 23.45 * np.sin(np.radians((360 / 365) * (dia - 81)))
                            decl_rad = np.radians(decl)
                            lat_rad = np.radians(lat)
                            argumento = np.clip(-np.tan(lat_rad) * np.tan(decl_rad), -1, 1)
                            h_s = np.arccos(argumento)
                            rad = S0 * Ta * (
                                np.cos(lat_rad) * np.cos(decl_rad) * np.sin(h_s) +
                                h_s * np.sin(lat_rad) * np.sin(decl_rad)
                            )
                            rad = max(0, rad)
                            if variable == 'Radiación Solar Corregida (W/m²)':
                                rad *= (1 + k * alt)
                            return rad

                        serie = pd.DataFrame({'ds': df['Fecha']})
                        serie['y'] = serie['ds'].apply(radiacion_diaria)
                    else:
                        return pd.DataFrame(columns=['ds', 'y'])

                    serie = serie.dropna(subset=['ds', 'y'])
                    # Si hay más de un registro el mismo día, usamos su promedio.
                    serie = serie.groupby('ds', as_index=False)['y'].mean().sort_values('ds')
                    return serie

                return pd.DataFrame(columns=['ds', 'y'])

            # Prophet usa aquí los registros diarios originales, no los promedios mensuales.
            df_estacion = cargar_serie_diaria_estacion(
                estacion_seleccionada, variable_seleccionada, output_dirs, elevation_data, tile_size
            )

            if df_estacion.empty:
                st.warning("No hay datos diarios válidos para la estación y variable seleccionadas.")
            else:
                fecha_min = df_estacion['ds'].min().date()
                fecha_max = df_estacion['ds'].max().date()
                fecha_max_pronostico = (df_estacion['ds'].max() + pd.Timedelta(days=365)).date()

                st.caption(
                    f"Datos observados: {fecha_min.strftime('%d/%m/%Y')} a {fecha_max.strftime('%d/%m/%Y')}. "
                    f"El módulo permite proyectar hasta 365 días después del último registro."
                )

                fecha_objetivo = st.date_input(
                    "Selecciona la fecha que quieres estimar",
                    value=min(fecha_max_pronostico, (df_estacion['ds'].max() + pd.Timedelta(days=180)).date()),
                    min_value=fecha_min,
                    max_value=fecha_max_pronostico,
                    format="DD/MM/YYYY"
                )
                fecha_objetivo_ts = pd.Timestamp(fecha_objetivo)

                if df_estacion['y'].notna().sum() < 10:
                    st.warning("No hay suficientes datos diarios válidos para entrenar un modelo Prophet confiable.")
                else:
                    with st.spinner("Entrenando modelo Prophet con datos diarios..."):
                        try:
                            # La estacionalidad anual es central para variables climatológicas.
                            modelo = Prophet(
                            ...
                            ultima_fecha = df_estacion['ds'].max().normalize()
                            dias_futuros = max(0, (fecha_objetivo_ts.normalize() - ultima_fecha).days)
                            futuro = modelo.make_future_dataframe(periods=dias_futuros, freq='D')

                            # Garantizar que la fecha objetivo esté incluida incluso si cae
                            # dentro del periodo histórico pero no existe un registro ese día.
                            if fecha_objetivo_ts.normalize() not in set(futuro['ds'].dt.normalize()):
                                futuro = pd.concat([
                                    futuro,
                                    pd.DataFrame({'ds': [fecha_objetivo_ts.normalize()]})
                                ], ignore_index=True).drop_duplicates(subset=['ds']).sort_values('ds')

                            predicciones = modelo.predict(futuro)

                            fila_objetivo = predicciones[
                                predicciones['ds'].dt.normalize() == fecha_objetivo_ts.normalize()
                            ].iloc[0]

                            estimado = float(fila_objetivo['yhat'])
                            inferior = float(fila_objetivo['yhat_lower'])
                            superior = float(fila_objetivo['yhat_upper'])

                            # Precipitación y evaporación no tienen interpretación física negativa.
                            if variable_seleccionada in ['Precipitación(mm)', 'Evaporación(mm)']:
                                estimado = max(0.0, estimado)
                                inferior = max(0.0, inferior)
                                superior = max(0.0, superior)

                            unidad = 'mm' if variable_seleccionada in ['Precipitación(mm)', 'Evaporación(mm)'] else (
                                '°C' if 'Temperatura' in variable_seleccionada else 'W/m²'
                            )

                            c1, c2, c3 = st.columns(3)
                            c1.metric(
                                f"Estimación para {fecha_objetivo.strftime('%d/%m/%Y')}",
                                f"{estimado:.2f} {unidad}"
                            )
                            c2.metric("Intervalo inferior", f"{inferior:.2f} {unidad}")
                            c3.metric("Intervalo superior", f"{superior:.2f} {unidad}")

                            observado = df_estacion[df_estacion['ds'].dt.normalize() == fecha_objetivo_ts.normalize()]
                            if not observado.empty:
                                st.info(
                                    f"En esa fecha también existe un dato observado: "
                                    f"{float(observado['y'].iloc[0]):.2f} {unidad}. "
                                    "Esto permite comparar observación y estimación."
                                )
                            elif fecha_objetivo_ts <= ultima_fecha:
                                st.info("La fecha está dentro del periodo histórico, pero no hay una observación directa ese día; se muestra la estimación del modelo.")
                            else:
                                st.success("La fecha está después del último registro disponible; se muestra un pronóstico temporal de Prophet.")
